...
export class Subcancertypes3Component implements OnInit {
  ...
  ngOnInit(): void {
    this.crumbs = [
      ...
      {label:'SubCancerTypes2',  icon: 'fa fa-plus', url: 'http://localhost:4200/subCancerTypes2' + '/' + this.routes.snapshot.params["id"]},
      {label:'SubCancerTypes3',  icon: 'fa fa-plus', url: this.route.url}
    ]
  }
